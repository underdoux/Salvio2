...
try {
    ...
    $sql = "INSERT IGNORE INTO investors 
            (name, email, percentage, status) 
            VALUES 
            ('John Doe', 'john@example.com', 60.00, 'active'),
            ('Jane Smith', 'jane@example.com', 40.00, 'active')";
    $db->exec($sql);
    ...
    // Get investor IDs
    $sql = "SELECT id, percentage FROM investors WHERE status = 'active' ORDER BY id LIMIT 2";
    $investors = $db->query($sql)->fetchAll(PDO::FETCH_ASSOC);

    // Insert distributions
    foreach ($investors as $investor) {
        $amount = (25000.00 * $investor['percentage']) / 100;
