...
try {
    ...
    foreach ($profits as $profit) {
        $sql = "INSERT INTO monthly_profits 
                ...
                SELECT :period, :total_sales, :total_product_cost, :total_commissions, :total_expenses, :net_profit, :status
                FROM dual
                WHERE NOT EXISTS (
                    SELECT 1 FROM monthly_profits WHERE period = :period
                )";
        $stmt = $db->prepare($sql);
        $stmt->execute($profit);
        if ($stmt->rowCount() > 0) {
            echo "Inserted profit record for {$profit['period']}\n";
        }
    }

    // Get January profit ID
    $sql = "SELECT id FROM monthly_profits WHERE period = '2024-01-01'";
